...
class TaskController extends Controller
{
    ...
    //     return view('sps.venue-admin.tasks.partials.index', compact('tasks', 'venues', 'categories'));
    // }
    ...
    //     return view('sps.venue-admin.tasks.partials.index', compact(
    //         'categories',
    //         'tasks',
    //         'currentVenue',
    //         'currentEvent',
    ...
    public function index()
    {
        ...
        if (! $isAssignedToEvent || ! $isAssignedToVenue) {
            return view('sps.venue-admin.tasks.partials.error_page', [
    ...
    //     $categories = Category::where('event', $eventId)
    //         ->get();

    //     // $tasks = LeadTask::where('venue_id', 4)->orderBy('created_at', 'desc')->get();
    //     $venues = Venue::orderBy('title')->get(); // fetch all venues
    //     $events = Event::orderBy('name')->get(); // fetch all events
    //     // Fetch current venue & event names
    //     $currentVenue = Venue::find($venueId);
    //     $currentEvent = Event::find($eventId);

    //     return view('sps.venue-admin.tasks.partials.index', compact('categories',  'venues', 'events', 'currentVenue', 'currentEvent'));
    // }
    ...
    public function toggle($id)
    {
        ...
        $categories = LeadCategory::where('event_id', $eventId)
        ...
        return view('sps.venue-admin.tasks.partials.list', compact('categories',  'currentUser', 'currentVenue', 'currentEvent'));
    }
    ...
    public function exportPdf()
    {
        ...
        // Load PDF view
        $pdf = Pdf::loadView('sps.venue-admin.tasks.partials.pdf', [
    ...
    //     $comment = LeadComment::updateOrCreate(
    //         ['venue_id' => $venueId, 'event_id' => $eventId, 'category_id' => $request->input('category_id')],
    //         ['comment' => $commentText]
    //     );

    //     return response()->json([
    //         'success' => true,
    //         'message' => 'Comment added to your tasks! Please refresh the page to see it.',
