...
use App\Models\Setting\PermissionVenueEvent;
...
class PermissionVenueEventController extends Controller
{
    ...
    {
        $permission_venue_events = PermissionVenueEvent::all();
        $venues = Venue::all();
        // dd($events);
        return view('setting.permission.venue_event.list', compact('permission_venue_events', 'venues'));
    }

    public function get($id)
    {
        $op = PermissionVenueEvent::findOrFail($id);
    ...
    public function list()
    {
        ...
        $ops = PermissionVenueEvent::orderBy($sort, $order);
        ...
        $ops = $ops->paginate(request("limit"))->through(function ($op) {
            ...
            // if ($op->event_logo) {
            //     $route_image = route('sps.setting.event.file', $op->event_logo);
            //     $image = ' <div class="avatar avatar-m">
            //                     <a  href="#" role="button" id="editPermVenueEvents" data-id=' . $op->id . ' data-table="event_table">
            //                         <img class="rounded-circle pull-up" src="' . $route_image . '" alt="" />
            //                     </a>
            //                 </div>';
            // } else {
            //     $image = '  <div class="avatar avatar-m  me-1" >
            //                     <a class="dropdown-toggle dropdown-caret-none d-inline-block" href="#" role="button" id="editPermVenueEvents" data-id=' . $op->id . ' data-table="event_table">
            ...
            $update_action =
                '<a href="javascript:void(0)" class="btn btn-sm" id="editPermVenueEvents" data-id=' . $op->id .
                ' data-table="perm_venue_event_table" data-bs-toggle="tooltip" data-bs-placement="right" title="Update">' .
                '<i class="fa-solid fa-pen-to-square text-primary"></i></a>';
            $delete_action =
                '<a href="javascript:void(0)" class="btn btn-sm" data-table="perm_venue_event_table" data-id="' .
                $op->id .
