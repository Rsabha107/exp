...
<!-- <script>
    $(function() {
        ...
        $('#tasks-list').on('change', '.task-toggle', function() {
            ...
            var url = '/sps/user/tasks/' + taskId + '/toggle';
        ...
        // Optional: handle delete via AJAX (same pattern)
        ...
        $('#createTaskForm').submit(function(e) {
            ...
            $.ajax({
                url: "{{ route('sps.user.tasks.store') }}", // ✅ use store route
                type: 'POST',
                data: form.serialize(),
                success: function(html) {
                    $('#createTaskModal').modal('hide');
                    toastr.success('Task created successfully!');
                    $('#tasks-list').html(html); // refresh only the list
                    form[0].reset();
                },
                error: function(xhr) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success && response.html) {
                                $('#tasks-list').html(response.html); // refresh the list
                                Swal.fire({
                                    icon: 'success',
                                    title: 'Deleted!',
